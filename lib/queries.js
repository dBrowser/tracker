...
exports.computeReport = async (db, date = undefined) => {
  ...
  console.log('Computing report', reportId, thisWeek)
  ...
  thisWeeksPings.forEach(ping => {
    var bucket = JSON.stringify({bench: ping.dBrowserVersion, os: ping.os})
    counts[bucket] = (counts[bucket] || 0) + 1
  })

  // store data
  await db.run(`INSERT INTO reports (id, activeUserCount, totalUserCount, computeDate) VALUES (?, ?, ?, ?)`, [reportId, thisWeeksPings.length, totalUserCount, Date.now()])
...
exports.addPing = async (db, data) => {
  ...
  // TODO this was deleting more than just today's pings... not sure why -prf
  ...
  var row = {
    ...
    dBrowserVersion: data.dBrowserVersion,
    os: data.os,
    ip: data.ip,
    date: Date.now()
  }
